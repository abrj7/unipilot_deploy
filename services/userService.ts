...
export const processUserInteraction = (text: string): { stats: UserStats, newBadges: Badge[] } => {
  ...
  const newLevel = Math.floor(stats.experience / 50) + 1;
  ...
  topics.forEach(topic => {
    if (topic.words.some(w => lowerText.includes(w))) {
      if (!stats.topicCounts) stats.topicCounts = {};
  ...
  if (stats.messagesCount >= 1 && !stats.badges.find(b => b.id === 'freshman')?.unlocked) {
  ...
  const uniqueTopics = stats.topicCounts ? Object.keys(stats.topicCounts).length : 0;
...
export const generateUserContextSummary = (stats: UserStats): string => {
  const favoriteTopic = stats.topicCounts ? Object.entries(stats.topicCounts)
